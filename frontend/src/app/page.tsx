...
export default function Home() {
  ...
  const handleReading = useCallback((payload: unknown) => {
    ...
    setDevice((current) => ({
      deviceId: reading.deviceId ?? current?.deviceId ?? "esp32-01",
      status: "online",
      lastSeen: timestamp,
      rssi: current?.rssi ?? -50,
    }));
  }, [device?.deviceId]);
  ...
  const handleStatus = useCallback((payload: unknown) => {
    const statusPayload = payload as { deviceId?: string; status: "online" | "offline"; lastSeen?: string; rssi?: number };
    setDevice((current) => ({
      deviceId: statusPayload.deviceId ?? current?.deviceId ?? "esp32-01",
      status: statusPayload.status,
      lastSeen: statusPayload.lastSeen ?? new Date().toISOString(),
      rssi: statusPayload.rssi ?? current?.rssi ?? -60,
    }));
  }, []);

  useSocket(handleReading, handleAnomaly, handleStatus);
