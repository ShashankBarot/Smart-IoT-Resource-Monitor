...
The frontend is a **Next.js 14+ App Router** single-page application that provides a real-time dashboard for monitoring IoT sensor data — specifically water flow (from a YF-S201 flow sensor) and electrical parameters (from a PZEM-004T sensor) — streamed via an ESP32 microcontroller through a Node.js backend.
...
|---|---|---|
| **Next.js** | `14.x` (latest) | App Router provides nested layouts, server components, and built-in file-system routing. The `app/` directory makes route grouping and shared layouts trivial. Next.js 14's partial pre-rendering and Turbopack dev server dramatically speeds up iteration. |
| **React** | `18.x` | Required by Next.js 14. React 18's concurrent features (Suspense, transitions) are leveraged for loading states. Hooks-based architecture keeps components clean and testable. |
...
| Package | Version | Reason for Choice |
|---|---|---|
| **Tailwind CSS** | `3.4+` | Utility-first CSS eliminates the overhead of maintaining a separate CSS file per component. JIT compilation keeps bundle sizes minimal. Tailwind's design tokens (colors, spacing, breakpoints) enforce visual consistency across all pages. |
