...
- [ ] **Mosquitto MQTT Broker** is running.
  - Verify: Open a dedicated terminal. Run `mosquitto -v -c mosquitto.conf` (or confirm the service is active). Keep this terminal **visible on screen** during the demo.
- [ ] **Backend (Node.js / Express / TypeScript)** is running.
  - Start with: `npm run dev` from the `backend/` directory.
  - Verify: Terminal shows `[Server] Listening on port 4000` and `[MQTT] Connected to broker`.
...
- [ ] **Browser** is open at `http://localhost:3000/dashboard`.
...
- [ ] **MQTT Simulator script** (`scripts/simulate_mqtt.py` or `scripts/simulate_mqtt.js`) is tested and ready to run.
...
- [ ] Phone hotspot credentials are saved and tested — know the SSID and password. ESP32 firmware `WIFI_SSID` and `WIFI_PASSWORD` can be changed in `firmware/src/config.h` if needed (requires re-flash — do this at home if needed).
